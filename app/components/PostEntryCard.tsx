...
export const PostEntryCard = (props: IBlogPostFields) => {
  return (
    <div className="group prose prose-a:no-underline prose-h3:my-0 prose-p:my-1 md:prose-h3:my-0 md:prose-p:my-1 w-full max-w-none">
      <Link href={`/posts/${props.slug}`} className="blog-entry-title">
        <div className="grid grid-cols-12 gap-x-4 gap-y-0 place-content-center trasition-all border-1 w-full gap-2 rounded-lg border bg-slate-50 p-4 shadow-none duration-300 ease-in-out hover:border-transparent hover:bg-white hover:shadow-lg">
          <div className="col-span-3 row-span-2 sm:col-span-2 flex align-center">
            {props.coverImage?.fields.file.url && (
              <Image
          ...
          <div className="col-span-9 row-span-2 sm:col-span-10 sm:row-span-1">
            ...
            >
              ...
              >
                <h3 className="blog-entry-title">{props["title"]}</h3>
                <p
                  className={
                    "blog-entry-date whitespace-nowrap text-sm text-slate-400"
                  }
                >
                  {props.created && formatDate(props.created)}
                </p>
              </div>
            </div>
          </div>
          <div className={"blog-entry-description col-span-12 md:col-span-10"}>
            <p>
              {props.description}
              <span className="flex items-center justify-end md:justify-start group-hover:text-sky-400 group-hover:translate-x-0.5 transition-colors transition-transform">
                <span>Read more</span>
